...
export class CppGenerator extends LanguageGenerator {
    readonly languageId: LanguageId = 'cpp';
    ...
    protected renderTextLine(text: string, escaped: string, mode: number, variant: number): TextEmit {
        switch (mode) {
            ...
            case 3: {
                ...
                const prefix = `${INDENT}std::string ${varName} = "`;
                return { line: `${prefix}${escaped}";`, textStart: prefix.length, textEnd: prefix.length + escaped.length };
            }
            default: { // 4
                const prefix = `${INDENT}std::cout << "`;
                return { line: `${prefix}${escaped}" << std::endl;`, textStart: prefix.length, textEnd: prefix.length + escaped.length };
            }
        }
    }
}
